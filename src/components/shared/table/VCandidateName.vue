...
<template lang="pug">
div.candidate-table-name
    span
      | {{ options.firstName}}  {{options.lastName }}
      svgicon(v-if="options.userStatus === 1" style="margin-left: 10px" name="global/candidate-approved" :fill="true" ref="svgicon" :original="true" v-tooltip.top-start="$t('candidates_sidebar.tooltip_verified')")
      span.candidate-table-name__resend-invitation(v-else  @click.stop="resendInvite" v-tooltip.top-start="$t('candidates_sidebar.tooltip_reinvite')")
